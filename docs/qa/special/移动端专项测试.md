...
### [关于移动应用](https://mp.weixin.qq.com/s/mh1pt-Wgw8CxNyrRpOkuKQ)
## 移动端APP生命周期
### android
...
1. 冷启动耗时
	- 方式一：录制冷启动视频，从桌面点击app开始至闪频页完全消失，使用FFmpeg拆帧
	- android启动时长测试方法
		- （1）adb命令
		```shell script
		...
		- （3）埋点
	- ios启动时长测试方法
		- （1）xcode developer tool，使用instruments的time profiler插件，[xcode下载](https://developer.apple.com/xcode/)
...
1. 获取方式
	- (1)读取linux proc文件系统（精确、方便自动化集成）
		...
		```
	- (2)第三方工具，如腾讯GT、网易Emagee，这些工具原理是调用android系统底层API完成数据统计
		- [GT官网](https://github.com/TencentOpen/GT)
	- (3)linux top命令（有误差，但方便获取）
		```shell script
...
7. 测试清单
	...
	压力测试后的内存使用情况
	:::
8. [内存泄露](https://mp.weixin.qq.com/s/Cr3UoksLcEjqUkpnJYonQA)
### 上下行流量
