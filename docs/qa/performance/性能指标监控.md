...
```shell script
top - 13:01:17 up 85 days,  2:45,  2 users,  load average: 0.03, 0.01, 0.00
Tasks: 442 total,   1 running, 441 sleeping,   0 stopped,   0 zombie
%Cpu(s):  0.0 us,  0.0 sy,  0.0 ni,100.0 id,  0.0 wa,  0.0 hi,  0.0 si,  0.0 st
MiB Mem :  32104.3 total,    569.9 free,  22476.6 used,   9057.9 buff/cache
MiB Swap:    923.3 total,    913.0 free,     10.2 used.   8857.7 avail Mem 
...
```
1. us(user) 列显示了用户进程所花费 CPU 时间的百分比。这个数值越高，说明用户进程消耗的 CPU 时间越多，可以用来分析代码中的 CPU 消耗热点。
2. sy(system) 列表示系统进程消耗的 CPU 时间百分比。
3. ni(nice) 列表示改变优先级的进程占用 CPU 的百分比。
4. id(idle) 列表示 CPU 处于空闲状态的时间百分比。
5. wa(?wait *) 列显示了 I/O 等待所占用的 CPU 时间的百分比，这里 wa 的参考值为 0.5，如果长期高于这个参考值，需要注意是否存在磁盘瓶颈。
6. hi(hardware interrupt) 列表示硬件中断占用 CPU 时间百分比。
7. si(software interrupt) 列表示软件中断占用 CPU 时间百分比。
8. st(?service time) 列表示当系统运行在虚拟机中时，当前虚拟机在等待 CPU 为它服务的时间。
...
4. 用户进程就是所有由你启动的进程
5. 关于nice，参考[https://linux.9iphp.com/](https://linux.9iphp.com/)和[https://www.linuxcool.com/](https://www.linuxcool.com/)
6. 在已经输入 top 的情况下再输入数字 1，可以查看 CPU 的核数和每个核的运行状态
```shell script
...
#### 定位消耗资源过多的进程（案例）
1. top找到消耗cpu高的进程id（pid）
2. top -p pid,然后输入大写的H，单独监控该进程，查看该进程下所有的线程
3. 查找消耗cpu高的线程id
4. 后续操作，比如java程序，jstack对线程dump等
### 内存
#### free
...
Swap:       0       0       0
```
1. total、used、free 它们分别是总的物理内存大小、已经被使用的物理内存和空闲的物理内存值是多少。
2. 为什么 free 值很低却未必代表内存达到瓶颈呢？这和 Linux 内核机制有关系，简单来说，内存空间会开辟 buffer 和 cache 缓冲区，对于物理内存来说，这都属于被使用过的内存。而应用需要内存时，如果没有可用的 free 内存，内核就会从缓冲区回收内存以满足要求，当 free 值很低的时候，如上代码中的 available 就能体现出缓冲区可用内存的大小，这个指标可以比较真实地反映出内存是否达到使用上限。
### 磁盘
#### iostat
```shell script
[root@JD ~]# iostat -x
