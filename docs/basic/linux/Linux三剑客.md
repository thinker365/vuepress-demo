...
```
## awk：数据切片
- awk的名字来源于三个作者的名字简称，如果为三剑客排个地位，那么awk绝对是老大，他甚至可以代替grep，awk不光支持正则，还可以对字段进行分段处理
- 在一定程度上awk可以替代grep，但它没有使用grep简洁
```shell script
awk '/[t]../' /tmp/test.txt
```
```shell script
...
awk --help
```
- 拓展阅读：RS、ORS、FS、OFS 的区别和联系，参考[https://www.jb51.net/article/52891.htm](https://www.jb51.net/article/52891.htm)


## sed：数据修改
- 严格来说：sed它是 stream editor 流式编辑器的简称，它是文本处理中非常有用的工具，能够完美的配合正则表达式使用，功能不同凡响。
```shell script
